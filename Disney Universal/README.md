...
| Disney hotel — **target split decided 8/26: AKV savanna Jan 26–28 + Polynesian studio Jan 28–30.** Front leg: DVC Rental Store request ARF-217165 open ($100 held) for AKV savanna, either building (Kidani Jan 26–28 was claimed away 8/25 before a member booked; Saratoga offered, declined). Back leg: Poly currently closed for 28/29 — `dvc_watcher.py` fires a dedicated high-priority alert the moment a Poly studio opens for both nights (tell broker to add/revise the request then). Watcher also still hunts the full ladder (savanna block/split, any deluxe 4-night block). If a member secures a leg: $ balance + e-sign within 24h of the reservation number. ~Nov 1: book Caribbean Beach cash fallback (refundable) for whatever's uncovered | Bot running; watch email; decide Nov 1 | ☐ |
